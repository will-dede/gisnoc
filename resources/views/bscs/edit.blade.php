...
@if(auth()->check() && (auth()->user()->role === 'network_lead' || auth()->user()->role === 'superadmin'))
    <div class="w-full max-w-md mx-auto mt-10">
        <form method="POST" action="{{ route('bscs.update', $bsc) }}" class="bg-white shadow rounded p-8">
            @csrf
            @method('PUT')
            <h1 class="text-2xl font-bold mb-6 text-center">Modifier le BSC</h1>
            @if(session('success'))
            ...
            @endif
            <div>
                <label for="nom_bsc" class="block font-medium text-sm text-gray-700">Nom du BSC <span class="text-red-500">*</span></label>
                <x-text-input id="nom_bsc" name="nom_bsc" type="text" class="block mt-1 w-full" :value="old('nom_bsc', $bsc->nom_bsc)" required autofocus />
