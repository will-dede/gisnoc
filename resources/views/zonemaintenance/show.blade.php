<x-app-layout>
    <div class="container mx-auto px-4 py-8">
        <div class="max-w-2xl mx-auto">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="space-y-4">
                            <div>
                                ...
                                <p class="mt-1 text-sm text-gray-900">{{ $zonemaintenance->id }}</p>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Nom de la zone</label>
                                <p class="mt-1 text-sm text-gray-900">{{ $zonemaintenance->nom_zone }}</p>
                            </div>
                        </div>
                        <div class="space-y-4">
                            <div>
                            ...
                            <div>
                                ...
                                <p class="mt-1 text-sm text-gray-900">{{ $zonemaintenance->updated_at->format('d/m/Y H:i') }}</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="mt-6">
                <a href="{{ route('zonemaintenance.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-500 hover:bg-gray-600 text-white font-bold rounded focus:outline-none focus:shadow-outline">
                    <i class="fas fa-arrow-left mr-2"></i> Retourner à la liste des zones
                </a>
            </div>
        </div>
    </div>
            ...
            <div class="mt-3 text-center">
                ...
                <div class="mt-2 px-7 py-3">
                    <p class="text-sm text-gray-500">
                        Êtes-vous sûr de vouloir supprimer la zone "{{ $zonemaintenance->nom_zone }}" ?
                    </p>
                </div>
                <div class="flex justify-center mt-4 space-x-4">
                    ...
                    <form action="{{ route('zonemaintenance.destroy', $zonemaintenance) }}" method="POST" class="inline">
                        @csrf
                        @method('DELETE')
