<x-app-layout>
    ...
    <div class="w-full max-w-2xl mx-auto mt-10">
        <form method="POST" action="{{ route('sites.store') }}" class="bg-white shadow rounded p-8">
            @csrf
            <h1 class="text-2xl font-bold mb-6 text-center">Ajouter un site</h1>
            @if(session('success'))
                <div class="bg-green-100 text-green-800 p-2 rounded mb-4" style="display:flex; justify-content:space-between">
                    {{ session('success') }}
                    <a href="{{ route('sites.index') }}" class="hover:underline">Retourner à la liste des sites</a>
                </div>
            @endif
            @if(session('error'))
            ...
            <div class="grid grid-cols-2 gap-4 mb-6">
                <div>
                    ...
                    <select id="transmission" name="transmission" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                        <option value="">Sélectionner le transmission</option>
                        <option value="fo" {{ old('transmission') == 'fo' ? 'selected' : '' }}>Fibre optique (FO)</option>
                        <option value="fh" {{ old('transmission') == 'fh' ? 'selected' : '' }}>Faisceau Hertzien (FH)</option>
                    </select>
                ...
                <div>
                    ...
                    <select id="type_site_id" name="type_site_id" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                        <option value="">Sélectionner le NodeType</option>
                        @foreach($type_sites as $type)
                            <option value="{{ $type->id }}" {{ old('type_site_id') == $type->id ? 'selected' : '' }}>{{ $type->nom_type_site }}</option>
                        @endforeach
                    </select>
            ...
            <div class="grid grid-cols-2 gap-4 mb-6">
                <div>
                    ...
                    <x-text-input id="cell2G" name="cell2G" type="text" class="block mt-1 w-full" :value="old('cell2G')" />
                    <x-input-error :messages="$errors->get('cell2G')" class="mt-2" />
                </div>
                <div>
                    <x-input-label for="cell3G" :value="'Cellule 3G'" />
                    <x-text-input id="cell3G" name="cell3G" type="text" class="block mt-1 w-full" :value="old('cell3G')" />
            ...
            <div class="grid grid-cols-2 gap-4 mb-6">
                <div>
                    ...
                    <x-text-input id="cell4G" name="cell4G" type="text" class="block mt-1 w-full" :value="old('cell4G')" />
                    <x-input-error :messages="$errors->get('cell4G')" class="mt-2" />
                </div>
                <div>
                    <x-input-label for="nodeName" :value="'Node Name'" />
                    <x-text-input id="nodeName" name="nodeName" type="text" class="block mt-1 w-full" :value="old('nodeName')" />
            ...
            <div class="grid grid-cols-2 gap-4 mb-6">
                <div>
                    ...
                    <x-text-input id="ip3G" name="ip3G" type="text" class="block mt-1 w-full" :value="old('ip3G')" />
                    <x-input-error :messages="$errors->get('ip3G')" class="mt-2" />
                </div>
                <div>
                    <x-input-label for="ip4G" :value="'IP 4G'" />
                    <x-text-input id="ip4G" name="ip4G" type="text" class="block mt-1 w-full" :value="old('ip4G')" />
            ...
            <div class="grid grid-cols-2 gap-4 mb-6">
                <div>
                    ...
                    <select id="bsc_id" name="bsc_id" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                        ...
                        @foreach($bscs as $bsc)
                            <option value="{{ $bsc->id }}" {{ old('bsc_id') == $bsc->id ? 'selected' : '' }}>{{ $bsc->nom_bsc }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select id="rnc_id" name="rnc_id" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                        ...
                        @foreach($rncs as $rnc)
                            <option value="{{ $rnc->id }}" {{ old('rnc_id') == $rnc->id ? 'selected' : '' }}>{{ $rnc->nom_rnc }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select id="region_id" name="region_id" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                        ...
                        @foreach($regions as $region)
                            <option value="{{ $region->id }}" {{ old('region_id') == $region->id ? 'selected' : '' }}>{{ $region->nom_region }}</option>
                        @endforeach
                    </select>
