<x-app-layout>
    <div class="container mx-auto px-4 py-8">
        <div class="max-w-2xl mx-auto">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="space-y-4">
                            <div>
                                ...
                                <p class="mt-1 text-sm text-gray-900">{{ $rnc->id }}</p>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Nom du RNC</label>
                                <p class="mt-1 text-sm text-gray-900">{{ $rnc->nom_rnc }}</p>
                            </div>
                        </div>
                        <div class="space-y-4">
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Date de création</label>
                                <p class="mt-1 text-sm text-gray-900">{{ $rnc->created_at->format('d/m/Y H:i') }}</p>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Dernière modification</label>
                                <p class="mt-1 text-sm text-gray-900">{{ $rnc->updated_at->format('d/m/Y H:i') }}</p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            @if($rnc->sites && $rnc->sites->count() > 0)
            ...
            @endif
            <div class="mt-6">
                <a href="{{ route('rncs.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-500 hover:bg-gray-600 text-white font-bold rounded focus:outline-none focus:shadow-outline">
            ...
            <div class="mt-3 text-center">
                ...
                <div class="mt-2 px-7 py-3">
                    <p class="text-sm text-gray-500">
                        Êtes-vous sûr de vouloir supprimer le RNC "{{ $rnc->nom_rnc }}" ?
                    </p>
                </div>
                <div class="flex justify-center mt-4 space-x-4">
                    ...
                    <form action="{{ route('rncs.destroy', $rnc) }}" method="POST" class="inline">
                        @csrf
                        @method('DELETE')
