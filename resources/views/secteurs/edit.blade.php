<x-app-layout>
    <div class="w-full max-w-md mx-auto mt-10">
        <form method="POST" action="{{ route('secteurs.update', $secteur) }}" class="bg-white shadow rounded p-8">
            @csrf
            @method('PUT')
            <h1 class="text-2xl font-bold mb-6 text-center">Modifier le secteur</h1>
            @if(session('success'))
            ...
            <div>
                ...
                <select id="frequence_id" name="frequence_id" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                    ...
                    @foreach($frequences as $frequence)
                        <option value="{{ $frequence->id }}" {{ old('frequence_id', $secteur->frequence_id) == $frequence->id ? 'selected' : '' }}>{{ $frequence->nom_freq }}</option>
                    @endforeach
                </select>
            ...
            <div class="mt-4">
                ...
                <x-text-input id="nom_secteur" name="nom_secteur" type="text" class="block mt-1 w-full" :value="old('nom_secteur', $secteur->nom_secteur)" required autofocus />
