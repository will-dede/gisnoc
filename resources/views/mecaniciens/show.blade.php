                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="space-y-4">
                            <div>
                                ...
                                <p class="mt-1 text-sm text-gray-900">{{ $mecanicien->id }}</p>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Nom</label>
                                <p class="mt-1 text-sm text-gray-900">{{ $mecanicien->nom_mecano }}</p>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Prénom</label>
                                <p class="mt-1 text-sm text-gray-900">{{ $mecanicien->prenom_mecano }}</p>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Téléphone</label>
                                <p class="mt-1 text-sm text-gray-900">{{ $mecanicien->tel_mecano }}</p>
                            </div>
                        </div>
                        <div class="space-y-4">
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Propriétaire</label>
                                <p class="mt-1 text-sm text-gray-900">{{ $mecanicien->est_proprietaire ? 'Oui' : 'Non' }}</p>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Zone de maintenance</label>
                                <p class="mt-1 text-sm text-gray-900">{{ $mecanicien->zoneMaintenance->nom_zone ?? '-' }}</p>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Date de création</label>
                                <p class="mt-1 text-sm text-gray-900">{{ $mecanicien->created_at->format('d/m/Y H:i') }}</p>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Dernière modification</label>
                                <p class="mt-1 text-sm text-gray-900">{{ $mecanicien->updated_at->format('d/m/Y H:i') }}</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="mt-3 text-center">
                ...
                <div class="mt-2 px-7 py-3">
                    <p class="text-sm text-gray-500">
                        Êtes-vous sûr de vouloir supprimer le mécanicien "{{ $mecanicien->nom_mecano }} {{ $mecanicien->prenom_mecano }}" ?
                    </p>
                </div>
                <div class="flex justify-center mt-4 space-x-4">
                    ...
                    <form action="{{ route('mecaniciens.destroy', $mecanicien) }}" method="POST" class="inline">
                        @csrf
                        @method('DELETE')
