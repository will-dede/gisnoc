<x-app-layout>
    <div class="w-full max-w-md mx-auto mt-10">
        <form method="POST" action="{{ route('mecaniciens.update', $mecanicien) }}" class="bg-white shadow rounded p-8">
            @csrf
            @method('PUT')
            <h1 class="text-2xl font-bold mb-6 text-center">Modifier le mécanicien</h1>
            @if(session('success'))
            ...
            <div>
                ...
                <x-text-input id="nom_mecano" name="nom_mecano" type="text" class="block mt-1 w-full" :value="old('nom_mecano', $mecanicien->nom_mecano)" required autofocus />
            ...
            <div class="mt-4">
                ...
                <x-text-input id="prenom_mecano" name="prenom_mecano" type="text" class="block mt-1 w-full" :value="old('prenom_mecano', $mecanicien->prenom_mecano)" required />
            ...
            <div class="mt-4">
                ...
                <x-text-input id="tel_mecano" name="tel_mecano" type="text" class="block mt-1 w-full" :value="old('tel_mecano', $mecanicien->tel_mecano)" required />
            ...
            <div class="mt-4">
                ...
                <select id="est_proprietaire" name="est_proprietaire" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                    <option value="0" {{ old('est_proprietaire', $mecanicien->est_proprietaire) == '0' ? 'selected' : '' }}>Non</option>
                    <option value="1" {{ old('est_proprietaire', $mecanicien->est_proprietaire) == '1' ? 'selected' : '' }}>Oui</option>
                </select>
            ...
            <div class="mt-4">
                ...
                <select id="zone_maintenance_id" name="zone_maintenance_id" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                    ...
                    @foreach($zones as $zone)
                        <option value="{{ $zone->id }}" {{ old('zone_maintenance_id', $mecanicien->zone_maintenance_id) == $zone->id ? 'selected' : '' }}>{{ $zone->nom_zone }}</option>
                    @endforeach
                </select>
